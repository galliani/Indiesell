...
RSpec.describe 'Customer visits store front page', type: :system, js: true do # 18 seconds
  subject { page }
  ...
  describe 'product listed' do
    ...
    it 'should display checkout button' do
      within("#product-#{sample_product.id}") do
        expect(subject).to have_selector '.buynow'
      end
    end
    ...
    it 'should display product infos' do
      within("#product-#{sample_product.id}") do
        ...
        expect(subject).to have_selector  '.product-price',
          text: "#{sample_product.price.currency.symbol}#{sample_product.price.to_i.to_s}"
      end
    end
  end
end